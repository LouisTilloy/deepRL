...
class Agent(object):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        next_values = self.sess.run(self.critic_prediction,
                                    feed_dict={self.sy_ob_no: next_ob_no})
        current_values = self.sess.run(self.critic_prediction,
                                    feed_dict={self.sy_ob_no: ob_no})
        adv_n = re_n + self.gamma * next_values * (1 - terminal_n) - current_values
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # otherwise the values will grow without bound.
        # YOUR CODE HERE
        for target_step in range(self.num_target_updates):
            next_values = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
            target_values = re_n + self.gamma * next_values * (1 - terminal_n)
            for grad_step in range(self.num_grad_steps_per_target_update):
                self.sess.run(self.critic_update_op, feed_dict={
                    self.sy_ob_no: ob_no,
                    self.sy_target_n: target_values,
                })
