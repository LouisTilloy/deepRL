...
def get_one_hot(class_idx, nb_classes):
    one_hot_vector = np.zeros((nb_classes,))
...
class ObservedPointEnv(Env):
    ...
    def reset(self):
        self.reset_task()
        one_hot_task = get_one_hot(self._task, len(self.tasks))
        self._state = np.concatenate((np.array([0, 0], dtype=np.float32), one_hot_task), axis=0)
